...
bool describe_item(item_def &item, function<void (string&)> fixup_desc)
{
    ...
    formatted_string fs_desc = formatted_string(desc);
    ...
    command_type action;
    char lastch;
    popup->on(UI::slots.key_event, [&](wm_event ev) {
...
public:
    ...
    int show()
    {
        ...
        spell_icon->set_tile(tile_def(tileidx_spell(m_spell), TEX_GUI));
        title_hbox->add_child(move(spell_icon));
#endif

        auto title = make_shared<UIText>();
        title->set_text(formatted_string(spell_title(m_spell)));
        title->set_margin_for_crt({0, 0, 0, 0});
        title->set_margin_for_sdl({0, 0, 0, 10});
        title_hbox->add_child(move(title));
        ...
        text->set_text(formatted_string::parse_string(trimmed_string(m_desc)));
        text->wrap_text = true;
        ...
        bool done = false;
        char lastch;
        popup->on(UI::slots.key_event, [&done, &lastch](wm_event ev) {
            if (ev.type != WME_KEYDOWN)
                return false;
...
#ifdef USE_TILE
    vector<tile_def> mons_tiles;
    _get_tiles_for_monster(&mi, mons_tiles);
    if (mons_tiles.size() > 0)
    {
        auto tiles_stack = make_shared<UIStack>();
        for (const auto &tile : mons_tiles)
        {
            auto icon = make_shared<UIImage>();
            icon->set_tile(tile);
            tiles_stack->add_child(move(icon));
